...
일일 자동화는 모니터링 실행 후 변경 감지 여부와 관계없이 `dashboard/data` 재생성과 커밋을
시도합니다. 실행 중 API 오류가 발생해도 DB에 실패 이력이 기록되고 export가 가능하면 GitHub
Pages 대시보드의 모니터링 이력과 최신 실행 시각에 반영됩니다.
