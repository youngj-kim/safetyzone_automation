...
처음부터 일반 모니터링으로 운영 현재 테이블에 반영하지 말고 API 호출량과 예상 건수를 확인한 뒤
전국 기준선을 생성한다.
...
`--baseline`은 snapshot/current를 갱신하지만 변경 이벤트와 알림을 만들지 않는다. 따라서 기준선일에
API 본 서버에 이미 존재하던 시설은 `신규`가 아니라 `현재`로 등록된다.

일반 모니터링 실행에서는 어느 한 시군구라도 응답이 0건이면 해당 실행 전체가 실패하며 기존 현재
데이터는 변경되지 않는다. 이는 부분 장애를 대량 삭제로 오인하는 것을 막는다. 단, 기준선 적재에서는
최신 법정동코드에는 존재하지만 보호구역 API가 아직 0건으로 응답하는 코드를 빈 지역으로 허용한다.

전국 수집은 호출량이 크므로 공공 API 429 제한을 피하기 위해 요청 간격을 넉넉하게 둔다.
